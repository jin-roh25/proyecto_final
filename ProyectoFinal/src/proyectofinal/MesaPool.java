...
public class MesaPool extends javax.swing.JLayeredPane {
    ...
    private Point2D randomSpotSearcher() {
        ...
        while (!disponible) { //¡¡¡¡¡¡rocordar ajustar los random para quedar dentro del area de juego!!!!!!!!!!!
            p = new Point2D.Double(jLabel1.getX() + Math.random() * jLabel1.getWidth(),
                    jLabel1.getY() + Math.random() * jLabel1.getHeight());
            disponible = true;
            for (Bola b : bolas) {
                if (P2DMath.getMagnitude(P2DMath.subtract(b.getLocation(), p)) < b.getRadio() * 2) {
                    disponible = false;
                }
            }
        }
